...
const ButtonCustom = ({ items, onItemClick, children }: ButtonCustomProps) => {
    const { isOpen, menuRef, toggleMenu, handleItemClick } = useButtonCustom(onItemClick)

    return (
        <div
            className="relative text-center cursor-pointer border-[2px] h-[52px] w-[52px] border-[#E54B65] rounded-full"
            ref={menuRef}
        >
