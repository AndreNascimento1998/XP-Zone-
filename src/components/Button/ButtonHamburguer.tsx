...
const ButtonHamburguer = ({ items, onItemClick }: ButtonHamburguerProps) => {
    const { isOpen, menuRef, toggleMenu, handleItemClick } = useButtonHamburguer(onItemClick)
    ...
    return (
        <div className="relative" ref={menuRef}>
            ...
            <div
                className={`absolute right-0 top-full transition-max-height duration-300 ease-out overflow-hidden ${isOpen ? 'max-h-50' : 'max-h-0 opacity-0'}`}
            >
                    ...
                    {items &&
                        items.map((item, index) => (
                            <li
                                ...
                                className="text-purple-900 border-b border-gray-700 break-words overflow-hidden"
                            >
                                {item.name}
                            </li>
                        ))}
