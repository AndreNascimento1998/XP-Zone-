...
const Home = () => {
    ...
    return (
        <div className={'flex flex-col gap-20'}>
            ...
            {gamesHighlight ? (
                <CardHighlights title={'Jogos em destaque'} games={gamesHighlight} isFavorite={true} />
            ) : null}
            {gamesXbox ? <CardHighlights title={'Jogos do Xbox'} games={gamesXbox} isFavorite={true} /> : null}
            {gamesPs.length ? (
                <CardHighlights title={'Jogos do Playstation'} games={gamesPs} isFavorite={true} />
            ) : null}
            {gamesNintendo.length ? (
                <CardHighlights title={'Jogos do Nintendo'} games={gamesNintendo} isFavorite={true} />
            ) : null}
            <SectionBanner />
        </div>
    )
